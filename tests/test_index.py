...
def test_patch_cannot_change_indexed_timestamp():
    ...
    repodata = {
        "packages": {filename: {"indexed_timestamp": 2000}},
        "packages.conda": {},
    }
    instructions = {"packages": {filename: {"indexed_timestamp": 1}}}
...
def test_index_of_updated_package(testing_workdir):
    ...
    with index_cache.db as db:
        for row in db.execute("SELECT path, index_json FROM index_json"):
            actual = json.loads(row["index_json"])
            assert actual.pop("indexed_timestamp") == indexed_timestamps[row["path"]]
            assert actual == json.loads(dummy_index_json)
...
def test_track_features(index_data):
    ...
    list(channel_index.cache_for_subdir("noarch").indexed_shards())
