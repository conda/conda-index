...
Placeholder for `conda-index ` release.
...
- [ ] Ensure release candidates are being successfully built (see `conda-canary/label/rc-conda-index-`)
...
Once the release PRs are filed, successful canary builds will be available on `/conda-index/files?channel=rc-conda-index-` for manual testing.
